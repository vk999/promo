<h3><i>Управление страницами новостей</i></h3>
<?php
...
/*
$dataProvider = $model->getPagesList();
$dataProvider = Pages::model()->findAll();
...
//                $criteria->order = 'pc.id, pc.lang';
                $criteria->join='right join pages_content pc ON t.id = pc.page_id';
//                $criteria->addCondition('pc.lang = :lang');
          $criteria->addCondition("`t`.`group_id` = 2");
                $criteria->params=array(':lang'=>'ru');
                $pagination = array('pageSize'=>20,);

$dataProvider = new CActiveDataProvider('Pages', array('criteria' => $criteria, 'pagination' => $pagination, ));
echo '<input type="hidden" name="pagetype" value="news">';
...
$this->widget('zii.widgets.grid.CGridView', array(
  ...
  'columns'=>array(
        ...
        array(
            'name' => 'Название 1',
            'value' => '" [".$data->pages_contents[0]->lang."] ".CHtml::link(Share::CheckName($data->pages_contents[0]->name), Yii::app()->createUrl("site/PageUpdate",array("id"=>$data->id, "lang"=>$data->pages_contents[0]->lang, "pagetype"=>"news")))',
            'type' => 'html',
            'htmlOptions' => array('class'=>'m_element'),
        ),
        array(
            'name' => 'Название 2',
            'value' => '" [".$data->pages_contents[1]->lang."] ".CHtml::link(Share::CheckName($data->pages_contents[1]->name), Yii::app()->createUrl("site/PageUpdate",array("id"=>$data->id, "lang"=>$data->pages_contents[1]->lang, "pagetype"=>"news")))',
            'type' => 'html',
        ),
	    array(
	      'class'=>'CButtonColumn',
